...
@section('content_header')
    <h1>{{trans('Fordítások')}}</h1>
@stop

@section('content')
    <div class="panel panel-transparent">
        <p>
        {{trans('Figyelem, a fordítások nem láthatóak addig, amíg nincsenek exportálva az app/lang-ba a "php artisan translation:export" paranccsal vagy a Közzététel gombbal.')}}</p>
        <div class="alert alert-success success-import" style="display:none;">
            <p>{{trans('Sikeres importálás, feldolgozva')}} <strong class="counter">N</strong> {{trans('elem! Frissítsd az oldalt, hogy láthasdd a változásokat!')}}</p>
        </div>
        <div class="alert alert-success success-find" style="display:none;">
            <p>{{trans('Fordítások keresése sikeres, találtunk')}} <strong class="counter">N</strong> {{trans('element!')}}</p>
        </div>
        <div class="alert alert-success success-publish" style="display:none;">
            <p>{{trans('Fordítások közzététele sikeres a megadott csoporthoz:')}} '<?= $group ?>'!</p>
        </div>
        ...
        <p>
        <?php if(!isset($group)) : ?>
        <form class="form-inline form-import" method="POST" action="<?= action('\App\Http\Controllers\TranslationController@postImport') ?>" data-remote="true" role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <select name="replace" class="form-control">
                <option value="0">{{trans('Hozzáfűzzük az új fordításokat')}}</option>
                <option value="1">{{trans('Meglévő fordítások cseréje')}}</option>
            </select>
            <button type="submit" class="btn btn-success"  data-disable-with="{{trans('Betöltés')}}..">{{trans('Importálás')}}</button>
        </form>
        <form class="form-inline form-find" method="POST" action="<?= action('\App\Http\Controllers\TranslationController@postFind') ?>" data-remote="true" role="form" data-confirm="{{trans('Biztosan elindítod a keresést? Minden fordítás bekerül az adatbázisba.')}}">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <p></p>
            <button type="submit" class="btn btn-info" data-disable-with="{{trans('Keresés')}}.." >{{trans('Fordítások keresése a fájlokban')}}</button>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
        <form class="form-inline form-publish" method="POST" action="<?= action('\App\Http\Controllers\TranslationController@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="{{trans('Biztosan közzéteszi a(z) ')}} '<?= $group ?>? {{trans('Ezzel felülírja a jelenlegi fordításokat!')}}">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >{{trans('Közzététel')}}</button>
            <a href="<?= action('\App\Http\Controllers\TranslationController@getIndex') ?>" class="btn btn-default">{{trans('Vissza')}}</a>
        </form>
        <?php endif; ?>
        </p>
        <form role="form">
        ...
        <?php if($group): ?>
        <form action="<?= action('\App\Http\Controllers\TranslationController@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <textarea class="form-control" rows="3" name="keys" placeholder="{{trans('Kulcs hozzáadása')}}"></textarea>
            <p></p>
            <input type="submit" value="Add keys" class="btn btn-primary">
        </form>
        <hr>
        <h4>{{trans('Összesen')}}: <?= $numTranslations ?>, {{trans('változott')}}: <?= $numChanged ?></h4>
        <table class="table">
            <thead>
            <tr>
                <th width="15%">{{trans('Kulcs')}}</th>
            ...
            <tr id="<?= $key ?>">
                ...
                <?php if($deleteEnabled): ?>
                <td>
                    <a href="<?= action('\App\Http\Controllers\TranslationController@postDelete', [$group, $key]) ?>" class="delete-key" data-confirm="Are you sure you want to delete the translations for '<?= $key ?>?"><span class="glyphicon glyphicon-trash"></span></a>
                </td>
                <?php endif; ?>
            </tr>
            <?php endforeach; ?>
        ...
        <?php else: ?>
        <p>{{trans('Válassz egy csoportot a fordításhoz. Ha nem látható a csoport, győződj meg róla, hogy importáltad a fordításokat.')}}
   </p>
    ...
    <script>
        $(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?= action('\App\Http\Controllers\TranslationController@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?= action('\App\Http\Controllers\TranslationController@getIndex') ?>';
                }
            });
